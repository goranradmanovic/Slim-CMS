...
$app->get('/xlsx', $authenticated(), function () use ($app) {
	...
	$users = $app->user->query('SELECT users.id,username,first_name,last_name,active,users_permissions.user_id,users_permissions.is_admin,users_permissions.is_moderator,users_permissions.can_post_topic FROM users LEFT JOIN users_permissions ON users.id = users_permissions.user_id')->get();
	...
	$xlsx->writeToStdOut(); //Pisanje u fajl i downloadanje u browser
